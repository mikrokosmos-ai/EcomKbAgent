...
- [ ] 引入 pytest 测试套件，覆盖节点级单测与 Pipeline 级集成测试
- [ ] 任务状态从内存态迁移到 Redis，支持多实例部署
- [ ] 增加 RAG 检索质量评测集与自动化评估报告
- [ ] 支持更多文档格式（Word、HTML、Excel 手册）
- [ ] 商品主体识别结果的人工校对与纠错界面
- [ ] 检索参数（混合权重、RRF k、TopK 阈值）配置化并支持 A/B 实验
- [ ] 图片理解结果缓存，减少重复导入的模型调用
- [ ] 前端工程化改造 + 移动端适配
